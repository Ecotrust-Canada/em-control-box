...
#include "DigitalCaptureManager.h"
/*#include "output.h"
...
CaptureManager::CaptureManager(EM_DATA_TYPE* _em_data, string _baseVideoDirectory):StateMachine(STATE_NOT_RUNNING, SM_EXCLUSIVE_STATES) {
    em_data = _em_data;
...
unsigned long CaptureManager::Start() {
    ...
    if(__ANALOG) {
        ...
        if(__EM_RUNNING && !(__OS_DISK_FULL)) {
            ...
            if(pid_encoder[HARDCODED_CAM_INDEX] == 0) {
                ...
                snprintf(fileName, sizeof(fileName), "%s/%s-cam%d.mp4", baseVideoDirectory.c_str(), date, HARDCODED_CAM_INDEX);
...
void CaptureManager::KillAndReapZombieChildren(bool dontBlock) {
    ...
    void CaptureManager::thr_IPCaptureLoop() {
        ...
        if(madeProgress && GetState() & STATE_STARTING && __EM_RUNNING && !(__OS_DISK_FULL)) {
